与 Java 相同，Kotlin 中的 Map 也用于保存 key-value 对。不同的是 Kotlin 中 Map 也分为可变和不可变 </br>
实际上 Kotlin 并没有真正为只币 4 平台实现任何 Map 集合类（只是通过别名借用了 Java 集合框架的类），
...
- mapOf(): 该函数返回一个不可变的 Map 集合，该函数接收 0 或多个 key-value 作为 Map 的元素。
- mutableMapOf(): 该函数返回可变的 MutableMap 集合。该函数可接受 0 个或多个 key-value 对，
...
- linkedMapOf（）：该函数返回可变的 LinkedHashMap 集合 。该函数可接受 0 个或多个
  ...
  注意事项：</br>
  如果开发者明确希望在程序中使用 LinkedHashMap 集合，则建议使用
...
- sortedMapOf()函数创建的 Map 集合会按照 key 大小对 key-value 对排序。
  ...
  征所决定的。
- Java 的 Set 集合其实是由 Map 集合所维护的，因此如果有某个 Set，那么就一定有对应的 Map 。
