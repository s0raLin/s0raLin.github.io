...
- 程序流程控制，match、while、if 等都可以根据模式是否与值匹配来决定程序的执行路径。
...
let 通过模式匹配机制将一个值绑定到新变量上，如果模式就是单个变量名的形式，那么它就属于不可反驳模式，即不存在匹配失败的情况。
...
- 对于实现了 Copy 语义的类型，所有权不会转移
- 对于实现了 Clone 语义的类型，解构操作会导致所有权被转移给新的变量绑定。
...
由于字符串支持 Clone 语义，而整数实现了 Copy 语义，因此 tuple.0 的所有权被转移了，而 tuple.1 的所有权没被转移。
解决办法是使用 ref 关键字，它会在模式匹配时创建对该值的引用
...
mut 关键字可以出现在模式中，在解构一个值时，默认是不可变的，但可以用 mut 关键字声明可变的绑定。
...
可以在模式中组合 ref 和 mut 来声明一个可变引用。此时 Rust 的可变性规则(同一时间只能存在一个可变引用)依然适用
...
当一个值没有实现 Copy 语义时，通过解构就会发生所有权转移。但是，如果在模式中使用\_忽略该值，所有权就不会发生转移。
...
> - 由于 tuple.0 被忽略了(\_)，没有进行任何绑定，所以所有权没有转移;
> - 对于 tuple.1,虽然后面使用\_b 进行了绑定，而且\_b 并没有使用，但 tuple.1 的值仍然被绑定到\_b 上，所有权发生了转移。

## 不可反驳模式

在 Rust 中，模式分为可反驳和不可反驳两种。
...
let 语句本身没有为匹配失败提供可替代方案。如果匹配失败，就必须提供后备计划，以指导编译器正确地编译。
...
前面我们使用了 if-let-else 这样的形式。这里的模式是可反驳的。
...
也可以在 while let 表达式中使用模式。

- 只要模式匹配到值，while 循环就会一直执行，直到模式不能匹配到值时终止。
- 通过模式匹配成功的值，只在 while 循环体内有效
...
for 表达式的实现基于迭代器和模式匹配机制，在每次迭代时，它会调用 next 方法。

- 只要 next 返回 Some(item)，迭代就会继续执行
- 一旦返回 None，迭代器就会终止。
...
在解构结构体时，还可以通过 field_name:binding_name 规则进行重命名
...
字面量对模式进行了细化，只有满足字面量的值的模式才能匹配。通过这种方式为模式匹配添加额外条件。由于 else 分支的存在，该模式是可反驳的。
...
match 表达式与模式的结合被广泛用于控制流，match 的每个分支左侧对应一个模式，当被测试的值与该模式匹配时，该分支就会被执行。
...
当一个模式由多个模式组合而成时，匹配守卫会应用到所有的模式。只有当模式被匹配且匹配守卫为 true 时，整个模式才会被匹配。
